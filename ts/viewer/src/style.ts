...
export function generateStyles(tokens: Tokens = defaultTokensLight, opts?: { dark?: boolean }): Stylesheet[] {
  const t = tokens;
  return [
    { selector: 'node', style: { 'label': 'data(label)', 'text-valign': 'center', 'text-halign': 'center', 'font-size': t.sizes.font, 'background-color': '#fff', 'border-color': '#ddd', 'border-width': 1 } },
    ...
    { selector: 'node[type = "module"]', style: { 'background-color': '#fafafa', 'border-width': 2 } },
